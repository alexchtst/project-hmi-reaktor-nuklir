...
class MainApp(QStackedWidget):
    ...
    def on_listen_digsilent_path(self, value):
        ...
        if self.pltnssytemprojetcname in list(self.user_data_dspf.keys()):
            self.user_data_dspf[self.pltnssytemprojetcname]['dspf_path'] = value
            self.user_data_dspf[self.pltnssytemprojetcname]['event_cases'] = self.eventcases
            self.user_data_dspf[self.pltnssytemprojetcname]['cases'] = self.cases
    ...
    def show_scenario(self):
        ...
        self.scenario.digsilent_path = self.user_data_dspf[self.pltnssytemprojetcname]['dspf_path']
        self.scenario.proj_name = self.pltnssytemprojetcname

    def show_loadflowactifity(self):
        self.setCurrentIndex(4)

        self.loadflow.ds_pf_path_signal.emit(self.user_data_dspf[self.pltnssytemprojetcname]['dspf_path'])
        self.loadflow.project_name_signal.emit(self.pltnssytemprojetcname)
        self.loadflow.caseses_signal.emit(self.user_data_dspf[self.pltnssytemprojetcname]['cases'])

    def show_dynamicctifity(self):
        self.setCurrentIndex(5)

        self.dynamic.ds_pf_path_signal.emit(self.user_data_dspf[self.pltnssytemprojetcname]['dspf_path'])
        self.dynamic.project_name_signal.emit(self.pltnssytemprojetcname)
        self.dynamic.caseses_signal.emit(self.user_data_dspf[self.pltnssytemprojetcname]['cases'])
        self.dynamic.casesesevent_signal.emit(self.user_data_dspf[self.pltnssytemprojetcname]['event_cases'])
