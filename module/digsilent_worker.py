...
class DigsilentWorker(QObject):
    ...
    def run_long_task(self):
        print(
            self.__digsilent_path, self.__proj_name, self.__case_name, self.__start_sim,
            self.__stop_sim, self.__sim_step, self.__start_fault, self.__stop_fault, self.__fault_type
        )
        ...
        try:
            ...
            with subprocess.Popen(
                ...
                for line in iter(prs.stdout.readline, ''):
                    ...
                    if "FINISHED" in line:
                        self.message.emit("FINISHED JOB")
                        break
                prs.wait()
                # prs.kill()
